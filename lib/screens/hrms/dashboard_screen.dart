...
class _DashboardScreenState extends State<DashboardScreen> {
                    ...
                    LayoutBuilder(builder: (context, constraints) {
                      if (constraints.maxWidth >= 900) {
                        return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Expanded(child: _calendarCard(context)),
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight,
          colors: [scheme.surface, scheme.primaryContainer.withOpacity(.28)]),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Row(children: [
            Container(width: 34, height: 34, decoration: BoxDecoration(shape: BoxShape.circle, color: scheme.primary.withOpacity(.10)), child: Icon(Icons.water_drop_rounded, size: 18, color: scheme.primary)),
            const SizedBox(width: 9),
            const Text('Calendar', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
            const Spacer(),
            Text(_date(selectedDate), style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: scheme.primary)),
          ]),
        ),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: scheme.primary.withOpacity(.07), borderRadius: BorderRadius.circular(12)),
          child: Row(children: [Icon(Icons.event_available_rounded, size: 18, color: scheme.primary), const SizedBox(width: 8), Expanded(child: Text('Selected: ${_date(selectedDate)}', style: const TextStyle(fontWeight: FontWeight.w600)))]),
        ),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: Stack(children: [
        ...
        Padding(padding: const EdgeInsets.all(20), child: Row(children: [
          Container(width: 54, height: 54, decoration: BoxDecoration(shape: BoxShape.circle, color: scheme.primary.withOpacity(.13)), child: Icon(Icons.water_drop_rounded, size: 29, color: scheme.primary)),
          const SizedBox(width: 14),
          ...
          if (MediaQuery.sizeOf(context).width >= 650)
            Container(padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 9), decoration: BoxDecoration(color: scheme.surface.withOpacity(.75), borderRadius: BorderRadius.circular(14)), child: Row(children: [Icon(Icons.auto_awesome_rounded, size: 17, color: scheme.primary), const SizedBox(width: 7), const Text('Live Overview', style: TextStyle(fontWeight: FontWeight.w700))])),
        ])),
      ]),
    );
  }
  ...
  Widget _metric(String title, String value, IconData icon) => Builder(builder: (context) {
    final scheme = Theme.of(context).colorScheme;
    return Container(
      decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [scheme.surface, scheme.primaryContainer.withOpacity(.34)]), borderRadius: BorderRadius.circular(18), border: Border.all(color: scheme.primary.withOpacity(.09)), boxShadow: [BoxShadow(color: scheme.primary.withOpacity(.035), blurRadius: 14, offset: const Offset(0, 5))]),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800)), const SizedBox(height: 2), Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: scheme.onSurfaceVariant))]),
      ]),
    );
  });
  ...
  Widget _section(BuildContext context, String title, IconData icon, Widget child) {
    final scheme = Theme.of(context).colorScheme;
    return Container(decoration: BoxDecoration(color: scheme.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: scheme.primary.withOpacity(.07))), padding: const EdgeInsets.fromLTRB(16, 15, 16, 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _progress(String label, int value, int total) {
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 12), child: Column(children: [Row(children: [Expanded(child: Text(label)), Text('$value / $total', style: const TextStyle(fontWeight: FontWeight.w700))]), const SizedBox(height: 6), LinearProgressIndicator(value: ratio, minHeight: 8, borderRadius: BorderRadius.circular(8))]));
  }
