...
class _AttendanceManagementScreenState extends State<AttendanceManagementScreen> {
  ...
  @override Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (_, __) => ListView(padding: const EdgeInsets.all(16), children: [
      _todayCard(), const SizedBox(height: 12), _calendar(), const SizedBox(height: 12),
      _history(), const SizedBox(height: 12), _approval(), const SizedBox(height: 12),
      _reportCard(),
    ]),
  );
  ...
  Widget _todayCard() {
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [const Icon(Icons.today, size: 28), const SizedBox(width: 10), Expanded(child: Text("Today's Attendance", style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800))), Text(_date(now))]),
      const SizedBox(height: 12), ...items.map((x) => ListTile(contentPadding: EdgeInsets.zero, leading: CircleAvatar(child: Text(x.key.name.substring(0,1))), title: Text(x.key.name), subtitle: Text(x.value == null ? 'Not punched in' : '${x.value!.status} • In ${_fmt(x.value!.punchIn)} • Out ${_fmt(x.value!.punchOut)}'), trailing: Wrap(children: [if (x.value?.punchIn == null) IconButton(tooltip:'Punch In', onPressed:()=>widget.service.punchIn(x.key.id), icon:const Icon(Icons.login)), if (x.value?.punchIn != null && x.value?.punchOut == null) IconButton(tooltip:'Punch Out', onPressed:()=>widget.service.punchOut(x.key.id), icon:const Icon(Icons.logout))]))),
    ])));
  }
  ...
  Widget _calendar() => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 8), Wrap(spacing: 6, runSpacing: 6, children: List.generate(DateTime(month.year,month.month+1,0).day, (i) { final d=DateTime(month.year,month.month,i+1); final count=widget.service.attendance.where((r)=>r.date.year==d.year&&r.date.month==d.month&&r.date.day==d.day).length; return Container(width:42,height:42,alignment:Alignment.center,decoration:BoxDecoration(border:Border.all(color:Theme.of(context).colorScheme.outlineVariant),borderRadius:BorderRadius.circular(8)),child:Text('${d.day}\n$count',textAlign:TextAlign.center,style:TextStyle(fontSize:11,fontWeight:count>0?FontWeight.w700:FontWeight.normal))); }))
  ])));

  Widget _history() => Card(child: ExpansionTile(leading: const Icon(Icons.history), title: const Text('Attendance History', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${records.length} attendance record(s)'), children: [if(records.isEmpty) const ListTile(title: Text('No attendance history yet.')), ...records.map((r) { final e=widget.service.employees.where((x)=>x.id==r.employeeId).firstOrNull; return ListTile(title:Text(e?.name ?? r.employeeId), subtitle:Text('${_date(r.date)} • ${r.status}\nIn ${_fmt(r.punchIn)} • Out ${_fmt(r.punchOut)} • ${_duration(r.workingTime)}'), isThreeLine:true, trailing:IconButton(tooltip:'Correct punch',onPressed:()=>_correct(r),icon:const Icon(Icons.edit_calendar))); })]));
  ...
  Widget _approval() => Card(child: ExpansionTile(leading: const Icon(Icons.fact_check), title: const Text('Attendance Approval', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Review submitted corrections'), children: [
    ...
    ...corrections.entries.map((x) { final approved=approvals[x.key]; return ListTile(title:Text(x.key),subtitle:Text(x.value),trailing:approved==null?Wrap(children:[IconButton(onPressed:()=>setState(()=>approvals[x.key]='Approved'),icon:const Icon(Icons.check)),IconButton(onPressed:()=>setState(()=>approvals[x.key]='Rejected'),icon:const Icon(Icons.close))]):Chip(label:Text(approved)); })
  ]));
