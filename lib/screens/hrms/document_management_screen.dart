...
class _DocumentManagementScreenState extends State<DocumentManagementScreen> {
  ...
  Future<void> _upload() async {
    ...
    final ok = await showDialog<bool>(context: context, builder: (c) => StatefulBuilder(builder: (c, setDialog) => AlertDialog(
      ...
      content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        DropdownButtonFormField<String>(value: category, decoration: const InputDecoration(labelText: 'Document Type', border: OutlineInputBorder()), items: const [DropdownMenuItem(value: 'Employee Document', child: Text('Employee Document')), DropdownMenuItem(value: 'Company Document', child: Text('Company Document'))], onChanged: (v) => setDialog(() => category = v!)),
  ...
  Future<void> _verify(int i) async {
    setState(() => docs[i]['verified'] = true);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Document marked as verified')));
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Wrap(spacing: 10, runSpacing: 10, children: [
        _stat('Employee Documents', employeeDocs, Icons.badge), _stat('Company Documents', companyDocs, Icons.business), _stat('Pending Verification', pending, Icons.verified_user_outlined), _stat('Expiry Alerts', expiry, Icons.warning_amber)
      ]),
      const SizedBox(height: 12),
      Card(child: ListTile(leading: const Icon(Icons.upload_file), title: const Text('Upload Document', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Add employee or company documents'), trailing: FilledButton.icon(onPressed: _upload, icon: const Icon(Icons.add), label: const Text('Upload')))),
      const SizedBox(height: 12),
  ...
  List<Widget> _list(String type) {
    final list = docs.asMap().entries.where((e) => e.value['type'] == type).toList();
    if (list.isEmpty) return [const ListTile(title: Text('No documents yet.'))];
    return list.map((e) => _tile(e.key, e.value)).toList();
  }

  List<Widget> _verification() {
    final list = docs.asMap().entries.toList();
    if (list.isEmpty) return [const ListTile(title: Text('No documents uploaded.'))];
    return list.map((e) => ListTile(leading: Icon(e.value['verified'] == true ? Icons.verified : Icons.pending), title: Text(e.value['name']), subtitle: Text(e.value['owner'].toString().isEmpty ? e.value['type'] : '${e.value['type']} • ${e.value['owner']}'), trailing: e.value['verified'] == true ? const Text('Verified') : TextButton(onPressed: () => _verify(e.key), child: const Text('Verify')))).toList();
  }

  List<Widget> _expiryList() {
    final list = docs.asMap().entries.where((e) => (e.value['expiry'] as String).isNotEmpty).toList();
    if (list.isEmpty) return [const ListTile(title: Text('No expiry dates added.'))];
    return list.map((e) => _tile(e.key, e.value, showDelete: false)).toList();
  }

  Widget _tile(int i, Map<String, dynamic> d, {bool showDelete = true}) => ListTile(leading: const Icon(Icons.description), title: Text(d['name']), subtitle: Text('${d['owner'].toString().isEmpty ? 'Company' : d['owner']} • ${d['expiry'].toString().isEmpty ? 'No expiry date' : 'Expires: ${d['expiry']}'}'), trailing: showDelete ? IconButton(onPressed: () => setState(() => docs.removeAt(i)), icon: const Icon(Icons.delete_outline)) : const Icon(Icons.warning_amber));
}
