          ...
          return Card(
            child: ListTile(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => CompanySettingsDetailScreen(
                    service: service,
...
class CompanySettingsDetailScreen extends StatefulWidget {
  ...
  const CompanySettingsDetailScreen({
    super.key,
...
class _CompanySettingsDetailScreenState extends State<CompanySettingsDetailScreen> {
  ...
  List<Map<String, String>> get settings {
    switch (widget.section) {
      case 0:
        return [
          {'title': 'Company Name', 'value': 'HRMS Management'},
          {'title': 'Country', 'value': 'India'},
          {'title': 'Currency', 'value': 'INR'},
          {'title': 'Timezone', 'value': 'Asia/Kolkata'},
        ];
      case 1:
        return [
          {'title': 'Head Office', 'value': 'Main Branch'},
          {'title': 'Pune Branch', 'value': 'Active'},
          {'title': 'Mumbai Branch', 'value': 'Active'},
          {'title': 'Branch count', 'value': '3'},
        ];
      case 2:
        return [
          {'title': 'Monday - Friday', 'value': 'Working'},
          {'title': 'Saturday', 'value': 'Weekly Off'},
          {'title': 'Sunday', 'value': 'Weekly Off'},
          {'title': 'Work hours', 'value': '9:00 AM - 6:00 PM'},
        ];
      case 3:
        return [
          {'title': 'Republic Day', 'value': '26 January'},
          {'title': 'Independence Day', 'value': '15 August'},
          {'title': 'Gandhi Jayanti', 'value': '2 October'},
          {'title': 'Custom holidays', 'value': 'Manage list'},
        ];
      case 4:
        return [
          {'title': 'Annual leave', 'value': '18 days'},
          {'title': 'Sick leave', 'value': '12 days'},
          {'title': 'Carry forward', 'value': 'Enabled'},
          {'title': 'Approval required', 'value': 'Yes'},
        ];
      case 5:
        return [
          {'title': 'Grace period', 'value': '10 minutes'},
          {'title': 'Late mark', 'value': 'After grace period'},
          {'title': 'Overtime', 'value': 'Approval required'},
          {'title': 'Attendance mode', 'value': 'Office + Remote'},
        ];
      case 6:
        return [
          {'title': 'Pay cycle', 'value': 'Monthly'},
          {'title': 'Payroll date', 'value': 'Last working day'},
          {'title': 'Overtime calculation', 'value': 'Configured'},
          {'title': 'Tax settings', 'value': 'Enabled'},
        ];
      case 7:
        return [
          {'title': 'Leave requests', 'value': 'Manager → HR'},
          {'title': 'Expense claims', 'value': 'Manager → Finance'},
          {'title': 'Payroll approval', 'value': 'Finance → Admin'},
          {'title': 'Attendance correction', 'value': 'Manager'},
        ];
      default:
        return [
          {'title': 'Leave alerts', 'value': 'Enabled'},
          {'title': 'Attendance alerts', 'value': 'Enabled'},
          {'title': 'Payroll alerts', 'value': 'Enabled'},
          {'title': 'Announcements', 'value': 'Enabled'},
        ];
    }
  }
  ...
  Future<void> save() async {
    try {
      await widget.service.backend.saveSettings(
        'section_${widget.section}',
        {
          'title': widget.title,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Settings saved to cloud')));
      }
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Saved locally; cloud sync unavailable')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final policy = widget.section == 2 || widget.section == 4 || widget.section == 5 || widget.section == 6;
        ...
        children: [
          Card(
            child: ListTile(
              ...
              subtitle: const Text('Company configuration'),
            ),
          ),
          if (profile)
            Card(
          ...
          if (widget.section == 8)
            Card(
              child: SwitchListTile(
                ...
                subtitle: const Text('Allow configured HRMS alerts'),
                value: enabled,
          ...
          if (policy)
            Card(
              child: Padding(
                ...
                child: Row(
                  children: [
                    const Expanded(child: Text('Policy status', style: TextStyle(fontWeight: FontWeight.w700))),
                    DropdownButton<String>(
                      value: option,
                      items: const ['Enabled', 'Disabled', 'Requires approval']
                          .map((e) => DropdownMenuItem(value: e, child: Text(e)))
                          .toList(),
                      onChanged: (v) => setState(() => option = v ?? option),
                    ),
                  ],
                ),
              ),
