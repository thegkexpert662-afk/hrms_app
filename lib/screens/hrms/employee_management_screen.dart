...
class _EmployeeManagementScreenState extends State<EmployeeManagementScreen> {
  ...
  void profile(Employee e) => showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (_) => _EmployeeProfile(employee: e, date: date, onEdit: () { Navigator.pop(context); addOrEdit(employee: e); }, onDelete: () { Navigator.pop(context); widget.service.removeEmployee(e.id); setState(() {}); }));
  ...
  @override Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (_, __) => ListView(padding: const EdgeInsets.all(16), children: [
      ...
      ...filtered.map((e) => Card(margin: const EdgeInsets.only(bottom: 10), child: ListTile(onTap: () => profile(e), leading: CircleAvatar(child: Text(e.name.isEmpty ? '?' : e.name[0].toUpperCase())), title: Text(e.name, style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text('${e.id} • ${e.designation}\n${e.department} • ${e.phone}'), isThreeLine: true, trailing: PopupMenuButton<String>(onSelected: (v) { if(v=='edit') addOrEdit(employee:e); if(v=='delete'){widget.service.removeEmployee(e.id); setState(() {});} }, itemBuilder: (_) => const [PopupMenuItem(value:'edit', child: Text('Edit')), PopupMenuItem(value:'delete', child: Text('Delete'))])))),
    ]),
  );
...
class _EmployeeFormState extends State<_EmployeeForm> {
  ...
  @override Widget build(BuildContext context)=>Padding(padding:EdgeInsets.only(bottom:MediaQuery.viewInsetsOf(context).bottom),child:DraggableScrollableSheet(initialChildSize:.92,maxChildSize:.98,minChildSize:.7,expand:false,builder:(_,scroll)=>Material(child:Form(key:form,child:ListView(controller:scroll,padding:const EdgeInsets.all(16),children:[Row(children:[Expanded(child:Text(widget.employee==null?'Add Employee':'Edit Employee',style:const TextStyle(fontSize:24,fontWeight:FontWeight.w800))),IconButton(onPressed:()=>Navigator.pop(context),icon:const Icon(Icons.close))]),section('Personal & Contact Details',[field('name','Full Name',required:true),field('phone','Mobile Number',required:true),field('email','Email'),field('gender','Gender'),field('dob','Date of Birth'),field('bloodGroup','Blood Group')]),section('Employment Details',[field('department','Department',required:true),field('designation','Designation',required:true),field('manager','Manager / Reporting'),DropdownButtonFormField(value:employment,decoration:const InputDecoration(labelText:'Employment Type',border:OutlineInputBorder()),items:const ['Full Time','Part Time','Contract','Intern'].map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(v)=>setState(()=>employment=v!),),const SizedBox(height:12),DropdownButtonFormField(value:status,decoration:const InputDecoration(labelText:'Status',border:OutlineInputBorder()),items:const ['Active','Inactive','On Notice'].map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(v)=>setState(()=>status=v!),),const SizedBox(height:12),ListTile(contentPadding:EdgeInsets.zero,title:const Text('Joining Date'),subtitle:Text('${joining.day}/${joining.month}/${joining.year}'),trailing:IconButton(icon:const Icon(Icons.calendar_month),onPressed:()async{final d=await showDatePicker(context:context,firstDate:DateTime(2000),lastDate:DateTime(2100),initialDate:joining);if(d!=null)setState(()=>joining=d);})]),section('Address',[field('address','House / Street / Address',lines:2),field('city','City'),field('state','State'),field('pinCode','PIN Code')]),section('Emergency Contact',[field('emergencyName','Contact Name'),field('emergencyRelation','Relationship'),field('emergencyPhone','Emergency Phone')]),section('Bank Details',[field('bankName','Bank Name'),field('accountNumber','Account Number'),field('ifsc','IFSC Code')]),section('Documents',[field('documentName','Document Name'),field('documentNumber','Document Number')]),section('Skills / Education / Experience',[field('skills','Skills',lines:2),field('education','Education',lines:2),field('experience','Experience',lines:2)]),const SizedBox(height:6),SizedBox(height:52,child:FilledButton.icon(onPressed:(){if(!form.currentState!.validate())return;final old=widget.employee;final e=Employee(id:old?.id??'EMP${DateTime.now().millisecondsSinceEpoch}',name:c['name']!.text.trim(),department:c['department']!.text.trim(),designation:c['designation']!.text.trim(),manager:c['manager']!.text.trim(),joiningDate:joining,employmentType:employment,status:status,phone:c['phone']!.text.trim(),email:c['email']!.text.trim(),gender:c['gender']!.text.trim(),dob:c['dob']!.text.trim(),address:c['address']!.text.trim(),city:c['city']!.text.trim(),state:c['state']!.text.trim(),pinCode:c['pinCode']!.text.trim(),emergencyName:c['emergencyName']!.text.trim(),emergencyRelation:c['emergencyRelation']!.text.trim(),emergencyPhone:c['emergencyPhone']!.text.trim(),bankName:c['bankName']!.text.trim(),accountNumber:c['accountNumber']!.text.trim(),ifsc:c['ifsc']!.text.trim(),documentName:c['documentName']!.text.trim(),documentNumber:c['documentNumber']!.text.trim(),bloodGroup:c['bloodGroup']!.text.trim(),skills:c['skills']!.text.trim(),education:c['education']!.text.trim(),experience:c['experience']!.text.trim());Navigator.pop(context,e);},icon:const Icon(Icons.save),label:Text(widget.employee==null?'Add Employee':'Save Changes')))]))));
}
