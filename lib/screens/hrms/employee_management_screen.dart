...
class _EmployeeManagementScreenState extends State<EmployeeManagementScreen> {
  ...
  void profile(Employee e) => showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (_) => _EmployeeProfile(employee: e, date: date, onEdit: () { Navigator.pop(context); addOrEdit(employee: e); }, onDelete: () { Navigator.pop(context); widget.service.removeEmployee(e.id); }));
...
class _EmployeeFormState extends State<_EmployeeForm> {
  ...
  Widget field(String key, String label, {bool required = false, int lines = 1}) => Padding(
    padding: const EdgeInsets.only(bottom: 12),
    child: TextFormField(controller: c[key], maxLines: lines, validator: required ? (v) => v == null || v.trim().isEmpty ? 'Required' : null : null, decoration: InputDecoration(labelText: label, border: const OutlineInputBorder())),
  );

  Widget section(String title, List<Widget> children) => Card(
    margin: const EdgeInsets.only(bottom: 12),
    child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 14), ...children])),
  );
  ...
  @override Widget build(BuildContext context) => Padding(
    ...
    child: DraggableScrollableSheet(
      ...
      builder: (_, scroll) => Material(
        child: Form(key: form, child: ListView(controller: scroll, padding: const EdgeInsets.all(16), children: [
