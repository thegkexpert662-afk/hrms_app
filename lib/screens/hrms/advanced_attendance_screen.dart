                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 6),
                      Text('Live GPS, geofence, network, device, QR and remote-work controls.'),
                    ],
                  ),
                ),
              ],
...
class _AdvancedAttendanceDetailScreenState extends State<AdvancedAttendanceDetailScreen> {
  ...
  Future<void> runAction() async {
    try {
      ...
      if (companyId == null) {
        throw StateError('Sign in and complete company setup first.');
      }
      ...
      switch (widget.title) {
        case 'GPS Attendance':
          ...
          final result = await security.validate(
            companyId: companyId,
        ...
        case 'Office Locations':
          ...
          await widget.service.backend.saveOfficeLocation(
            id: DateTime.now().millisecondsSinceEpoch.toString(),
            name: nameController.text.trim().isEmpty ? 'Office' : nameController.text.trim(),
            latitude: position!.latitude,
            longitude: position!.longitude,
            radiusMeters: double.tryParse(valueController.text.trim()) ?? 150,
          );
        ...
        case 'IP Restriction':
          ...
          await widget.service.backend.saveSettings(
            'attendance',
            {
              'allowedIps': [valueController.text.trim().isEmpty ? ip : valueController.text.trim()],
        ...
        case 'Device Restriction':
          ...
          await widget.service.backend.db
              .collection('companies')
              .doc(companyId)
              .collection('employees')
              .doc(employee.id)
              .set({'allowedDeviceId': id}, SetOptions(merge: true));
        ...
        case 'QR Attendance':
          ...
          await Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => QrAttendanceScreen(service: widget.service)),
          );
          return;
        default:
          if (employee == null) throw StateError('No employee profile available.');
  ...
  String get hint {
    switch (widget.title) {
      case 'Office Locations':
        return 'Radius in meters, e.g. 150';
      case 'IP Restriction':
        return 'Approved IP, e.g. 192.168.1.10 or CIDR';
      case 'Device Restriction':
        return 'Tap Register Device';
      case 'Geofencing':
        return 'Uses saved office locations';
      default:
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          if (widget.title == 'Office Locations' || widget.title == 'IP Restriction') ...[
            TextField(
            ...
            TextField(
              ...
              keyboardType: widget.title == 'Office Locations' ? TextInputType.number : TextInputType.text,
          ...
          FilledButton.icon(
            onPressed: runAction,
            icon: Icon(widget.title == 'QR Attendance' ? Icons.qr_code_scanner : Icons.play_arrow),
            label: Text(
              widget.title == 'Device Restriction'
                  ? 'Register Device'
                  : widget.title == 'GPS Attendance'
                      ? 'Mark GPS Attendance'
                      : widget.title == 'QR Attendance'
                          ? 'Open QR Scanner'
                          : 'Apply / Save',
            ),
          ),
          if (message.isNotEmpty)
            Padding(
              ...
              child: Card(child: Padding(padding: const EdgeInsets.all(16), child: Text(message))),
            ),
          if (position != null)
            Padding(
