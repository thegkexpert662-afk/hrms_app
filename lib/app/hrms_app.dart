...
import '../services/hrms_backend_service.dart';
...
class Employee {
  ...
  Employee({required this.id, required this.name, required this.department, required this.designation, required this.manager, required this.joiningDate, required this.employmentType, required this.status, required this.phone, required this.email, this.gender = '', this.dob = '', this.address = '', this.city = '', this.state = '', this.pinCode = '', this.emergencyName = '', this.emergencyRelation = '', this.emergencyPhone = '', this.bankName = '', this.accountNumber = '', this.ifsc = '', this.documentName = '', this.documentNumber = '', this.bloodGroup = '', this.skills = '', this.education = '', this.experience = ''});
}
...
class HrmsService extends ChangeNotifier {
  final HrmsBackendService backend = HrmsBackendService();
  ...
  Future<void> loadFromCloud() async {
    try { await backend.hydrate(this); notifyListeners(); } catch (_) { /* Offline/local-first fallback. */ }
  }
  ...
  AttendanceRecord punchIn(String id, {bool wfh = false}) {
    final r = todayFor(id) ?? AttendanceRecord(employeeId: id, date: DateTime.now(), wfh: wfh);
    r.punchIn ??= DateTime.now(); r.wfh = wfh; r.status = DateTime.now().hour >= 10 ? 'Late' : 'Present';
    if (!attendance.contains(r)) attendance.add(r);
    notifyListeners();
    backend.saveAttendance(r).catchError((_) {});
    return r;
  }

  void punchOut(String id) { final r = todayFor(id); if (r?.punchIn != null && r?.punchOut == null) { r!.punchOut = DateTime.now(); r.status = r.workingTime < const Duration(hours: 8) ? 'Early Exit' : r.status; notifyListeners(); backend.saveAttendance(r).catchError((_) {}); } }

  void addEmployee(Employee e) { employees.add(e); notifyListeners(); backend.saveEmployee(e).catchError((_) {}); }
  void updateEmployee(Employee e) { final i = employees.indexWhere((x) => x.id == e.id); if (i >= 0) employees[i] = e; notifyListeners(); backend.saveEmployee(e).catchError((_) {}); }
  void removeEmployee(String id) { employees.removeWhere((e) => e.id == id); notifyListeners(); backend.deleteEmployee(id).catchError((_) {}); }
  void applyLeave(LeaveRequest r) { leaves.add(r); notifyListeners(); backend.saveLeave(r).catchError((_) {}); }
  void decideLeave(String id, bool ok) { final r = leaves.firstWhere((x) => x.id == id); r.status = ok ? 'Approved' : 'Rejected'; if (ok) leaveBalances[r.employeeId] = (leaveBalances[r.employeeId] ?? 0) - r.days; notifyListeners(); backend.updateLeaveStatus(id, r.status).catchError((_) {}); }
