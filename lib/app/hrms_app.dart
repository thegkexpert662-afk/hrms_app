...
class LeaveRequest {
  final String id, employeeId, type, reason; final DateTime from, to; String status;
...
class PayrollRecord {
  final String employeeId; final double basic, allowances, bonus, overtime, deductions;
  ...
  double get gross => basic + allowances + bonus + overtime; double get net => gross - deductions;
}
class HrmsService extends ChangeNotifier {
  ...
  void addEmployee(Employee e){employees.add(e);notifyListeners();} void updateEmployee(Employee e){final i=employees.indexWhere((x)=>x.id==e.id);if(i>=0)employees[i]=e;notifyListeners();} void removeEmployee(String id){employees.removeWhere((e)=>e.id==id);notifyListeners();}
  void applyLeave(LeaveRequest r){leaves.add(r);notifyListeners();} void decideLeave(String id,bool ok){final r=leaves.firstWhere((x)=>x.id==id);r.status=ok?'Approved':'Rejected';if(ok)leaveBalances[r.employeeId]=(leaveBalances[r.employeeId]??0)-r.days;notifyListeners();}
  int get present=>employees.where((e)=>todayFor(e.id)?.status=='Present').length; int get late=>employees.where((e)=>todayFor(e.id)?.status=='Late').length; int get earlyExit=>employees.where((e)=>todayFor(e.id)?.status=='Early Exit').length; int get wfh=>employees.where((e)=>todayFor(e.id)?.wfh==true).length; int get pendingLeaves=>leaves.where((l)=>l.status=='Pending').length;
...
class HrmsApp extends StatelessWidget {final HrmsService service; const HrmsApp({super.key,required this.service});@override Widget build(BuildContext c)=>MaterialApp(debugShowCheckedModeBanner:false,title:'HRMS Management System',theme:ThemeData(useMaterial3:true,colorScheme:ColorScheme.fromSeed(seedColor:const Color(0xFF2563EB)),scaffoldBackgroundColor:const Color(0xFFF5F7FB)),home:HrmsHome(service:service));}
